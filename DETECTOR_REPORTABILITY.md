...
For a struct introduction, the same rule requires a balanced containing declaration and a first named field with a plain identifier or qualified path type. The selected window must stop before a second field begins or the declaration closes. Documentation, outer attributes, and field visibility belong to that introduction. More complex first-field types, computed types, malformed declarations, and windows reaching later fields receive no additional suppression. The source-context scan excludes macro and attribute input, and quoted-continuation tracking excludes embedded raw-string programs even when blank lines create separate token segments. This refinement leaves line-stanza admission, complete field-table detection, token identities, and fingerprints unchanged. `struct_introductions_stop_before_a_second_field`, `struct_introductions_require_balanced_plain_field_context`, and `struct_introductions_preserve_embedded_program_windows` pin these boundaries; the shared rule-toggle regression preserves both complete candidate populations.
